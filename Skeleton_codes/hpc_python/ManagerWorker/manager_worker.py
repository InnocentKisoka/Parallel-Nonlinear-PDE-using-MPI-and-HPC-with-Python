...
def manager(comm, tasks):
    ...
    return patches, ranktimes_recv, ranktimes_work, TasksDoneByWorker
...
def perform(comm, nx, ny, ntasks):
    ...
    timespent = -MPI.Wtime()  # Start timing
    ...
    if my_rank == 0:  # If manager
        M = mandelbrot(x_min, x_max, nx, y_min, y_max, ny, ntasks)
        tasks = M.get_tasks()
        print("starting manager...")
        
        # Update this line to capture the additional return value
        patches, times_recv, times_work, TasksDoneByWorker = manager(comm, tasks)

        m = M.combine_tasks(patches)
        print("done combining patches")

        # Process imbalance metrics and task information
        timespent += MPI.Wtime()  # Stop timing
        ...
        print(f"{size} {nx} {ny} {ntasks} {maxes_recv:.3f} {maxes_work:.3f} {1-imb_recv:.3f} {1-imb_work:.3f}")
        for i in range(size):
            if i == MANAGER:
                continue
...
if __name__ == "__main__":
    ...
    if my_rank == 0:
        ...
        print(f"Average imbalance (recv): {avg_recv:.3f}")
        print(f"Average imbalance (work): {avg_work:.3f}")
